...
class ReconciliationHandler {
    ...
    std::unordered_map<NodeId, ReconState> m_states GUARDED_BY(m_states_mutex);
    ...
    static Optional<std::pair<Minisketch, uint16_t>> ParseRemoteSketch(const ReconState& recon_state,
    ...
    */
    ...
    Optional<ReconState> GetPeerState(const NodeId peer_id) const
    {
    ...
    CSerializedNetMsg PrepareAnnouncement(const NodeId peer_id, bool inbound, int common_version)
    {
        // Can't be announced more than once.
        LOCK(m_states_mutex);
        auto recon_state = m_states.find(peer_id);
        assert(recon_state == m_states.end());

        const CNetMsgMaker msg_maker(common_version);
        ...
        return msg_maker.Make(NetMsgType::SENDRECON, be_recon_requestor, be_recon_responder,
            recon_version, m_local_recon_salt);
    }

    bool EnableReconciliationSupport(const NodeId peer_id, bool inbound, bool recon_requestor,
        bool recon_responder, uint32_t recon_version, uint64_t remote_salt,
        size_t outbound_flooders)
    {
        ...
        m_states.emplace(peer_id, ReconState(recon_requestor, recon_responder,
    ...
    // - RECONCILDIFF if reconciliation is over (either after bisection or after init sketch)
    // - REQSKETCHEXT if current reconciliation round failed due to small sketch capacity.
    Optional<std::pair<CSerializedNetMsg, std::vector<uint256>>> HandleSketch(
        const NodeId peer_id, int common_version, std::vector<uint8_t>& skdata)
    {
        ...
        if (remote_sketch_capacity == 0 || !remote_sketch || !local_sketch) {
            ...
            return std::make_pair(msg_maker.Make(NetMsgType::RECONCILDIFF, false, std::vector<uint256>()), remote_missing);
        }
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            recon_state->second.UpdateOutgoingPhase(RECON_NONE);
            return std::make_pair(msg_maker.Make(NetMsgType::RECONCILDIFF, true, local_missing), remote_missing);
        } else {
            ...
            if (recon_state->second.GetOutgoingPhase() == RECON_INIT_REQUESTED) {
                ...
                return std::make_pair(msg_maker.Make(NetMsgType::REQSKETCHEXT), std::vector<uint256>());
            } else {
                ...
                recon_state->second.FinalizeOutgoingReconciliation(false, DEFAULT_RECON_Q);
                recon_state->second.UpdateOutgoingPhase(RECON_NONE);
                return std::make_pair(msg_maker.Make(NetMsgType::RECONCILDIFF, false, std::vector<uint32_t>()), remote_missing);
            }
        }
    }
