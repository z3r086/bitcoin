        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        if (m_txreconciliation->IsPeerRegistered(pto->GetId())) {
                            bool flood_target = m_txreconciliation->ShouldFloodTo(pto->GetId(), wtxid);
                            ...
                            for (const CTxMemPoolEntry& parent : parents) {
                                ...
                                if (m_txreconciliation->IsAlreadyInPeerSet(pto->GetId(), parent_wtxid) ||
                                    ...
                                    // 1. If the parent is in the reconciliation set, the two
