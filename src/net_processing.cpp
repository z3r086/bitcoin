...
public:
    ...
    void RelayTransaction(const uint256& txid, const uint256& wtxid, std::optional<NodeId> from=std::nullopt) EXCLUSIVE_LOCKS_REQUIRED(cs_main) override;
...
void PeerManagerImpl::FinalizeNode(const CNode& node, bool& fUpdateConnectionTime)
{
    ...
    m_out_flood_to -= state->m_we_may_flood_to;
    assert(m_out_flood_to >= 0);
        ...
        if (AlreadyHaveTx(GenTxid(/* is_wtxid=*/true, wtxid))) {
            if (pfrom.HasPermission(PF_FORCERELAY)) {
                ...
                if (!m_mempool.exists(tx.GetHash())) {
                    ...
                    RelayTransaction(tx.GetHash(), tx.GetWitnessHash(), pfrom.GetId());
                }
            }
            return;
        }
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    if (!pto->m_tx_relay->fRelayTxes) pto->m_tx_relay->m_txs_to_announce.clear();
                }
                ...
                if (fSendTrickle && pto->m_tx_relay->fSendMempool) {
                    ...
                    for (const auto& txinfo : vtxinfo) {
                        ...
                        pto->m_tx_relay->m_txs_to_announce.erase(hash);
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        pto->m_tx_relay->m_txs_to_announce.erase(it);
        ...
        //
        {
            ...
            if (respond) {
                // Note that it's possible and valid behaviour to send an empty sketch here, just
                // to let the peer know that we have nothing for them.
