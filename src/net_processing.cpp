                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    // Received from a peer demonstrating readiness to do reconciliations.
    // Tx reconciliation feature negotiation should happen between VERSION and
    ...
    if (msg_type == NetMsgType::SKETCH) {
        ...
        const auto recon_result_data = m_reconciliation.HandleSketch(pfrom.GetId(), pfrom.GetCommonVersion(), skdata);
        if (recon_result_data) {
            const auto [recon_done, recon_result, locally_missing_txs, remotely_missing_txs] = *recon_result_data;
            if (recon_done) {
                m_connman.PushMessage(&pfrom, msgMaker.Make(NetMsgType::RECONCILDIFF,
                    recon_result, locally_missing_txs));
                AnnounceTxs(remotely_missing_txs, pfrom);
            } else {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        if (supports_recon) {
                            // Check if reconciliation set is not at capacity.
                            ...
                            if (txs_to_reconcile.size() + recon_set_size < MAX_PEER_TX_ANNOUNCEMENTS) {
                                ...
                                // 2) the transaction isn't set for flooding based on its source
