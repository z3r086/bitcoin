                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (AcceptToMemoryPool(m_mempool, state, ptx, &lRemovedTxn, false /* bypass_limits */)) {
            ...
            bool flood = WITH_LOCK(peer->m_recon_state_mutex, return !(peer->m_recon_state && peer->m_recon_state->IsResponder()));
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        if (!peer->m_recon_state || (peer->m_recon_state->IsChosenForFlooding() && flood_tx)) {
                    ...
                    if (peer->m_recon_state && txs_to_reconcile.size() != 0) {
                        auto overflown_transactions = peer->m_recon_state->AddToReconSet(txs_to_reconcile, MAX_PEER_TX_ANNOUNCEMENTS);
                        if (overflown_transactions.size() > 0) {
                            ...
                            AnnounceTxs(overflown_transactions, *pto);
                        }
                    }
                }
            }
        ...
        //
        {
            ...
            if (peer->m_recon_state) {
                ...
                if ((peer->m_recon_state->GetIncomingPhase() == RECON_INIT_REQUESTED && current_time > peer->m_recon_state->GetNextRespond()) ||
                    peer->m_recon_state->GetIncomingPhase() == RECON_EXT_REQUESTED) {
                    Minisketch sketch;
                    std::vector<unsigned char> response_skdata;
                    if (peer->m_recon_state->GetIncomingPhase() == RECON_INIT_REQUESTED) {
                        ...
                        size_t lower_bytes_to_drop = peer->m_recon_state->GetCapacitySnapshot() * BYTES_PER_SKETCH_CAPACITY;
