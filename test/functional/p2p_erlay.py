...
from test_framework.messages import (
    ...
    msg_sketch, msg_reqbisec, msg_reconcildiff,
    CTransaction, CInv
)
...
class ReconciliationTest(BitcoinTestFramework):
    ...
    def transmit_sketch(self, test_node, txs_to_sketch, bisection, capacity, expected_announced_txs):
    ...
    def handle_reconciliation_finalization(self, test_node, expected_success, expected_requested_txs, after_bisection=False):
    ...
    def handle_bisection_request(self, test_node):
        def received_reqbisec():
            return (len(test_node.last_reqbisec) == 1)
        self.wait_until(received_reqbisec, timeout=30)
        test_node.last_reqbisec = []


    def request_bisection(self, test_node, expected_bisec_sketch):
    ...
    def test_incoming_recon(self):
        ...
        self.finalize_reconciliation(self.test_node, success=ReconResult.RECON_SUCCESS, txs_to_request=[], txs_to_expect=[])
        ...
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_SUCCESS, txs_to_request, txs_to_expect=txs_to_request)
        ...
        self.request_bisection(self.test_node, expected_sketch)
        txs_to_request = random.sample(remote_txs, 3)
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_SUCCESS, txs_to_request, txs_to_expect=txs_to_request)
        self.request_transactions(self.test_node, txs_to_request)
        self.check_remote_recon_set_incoming(more_remote_txs)
        ...
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.proceed_in_time(PENALIZED_INVENTORY_BROADCAST_INTERVAL + 1)
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_FAILED, txs_to_request=[], txs_to_expect=remote_txs)
        ...
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_LOW_FAILED, txs_to_request, txs_to_expect)
        self.request_transactions(self.test_node, txs_to_request)
        self.check_remote_recon_set_incoming(more_remote_txs)
        ...
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_HIGH_FAILED, txs_to_request, txs_to_expect)
        self.request_transactions(self.test_node, txs_to_request)
        self.check_remote_recon_set_incoming(more_remote_txs)
        ...
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.proceed_in_time(PENALIZED_INVENTORY_BROADCAST_INTERVAL + 1)
        self.finalize_reconciliation(self.test_node, ReconResult.RECON_SUCCESS, txs_to_request=[], txs_to_expect=[])
        self.check_remote_recon_set_incoming(more_remote_txs)
    ...
    def test_outgoing_recon(self):
        ...
        self.transmit_sketch(self.test_node, txs_to_sketch=local_txs, bisection=False,
            capacity=0, expected_announced_txs=remote_unique_txs)
        self.handle_reconciliation_finalization(self.test_node, expected_success=ReconResult.RECON_FAILED,
        ...
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.handle_reconciliation_finalization(self.test_node, expected_success=ReconResult.RECON_SUCCESS,
            expected_requested_txs=local_unique_txs, after_bisection=False)
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.request_transactions(self.test_node, remote_unique_txs)
        ...
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.handle_bisection_request(self.test_node)
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.transmit_sketch(self.test_node, txs_to_sketch=local_txs, bisection=True,
        ...
        local_txs, remote_txs, _ = self.generate_txs(10, 30, 0, blocks, send=False)
        while len(local_txs) != 30:
            tx = self.generate_txs(1, 0, 0, blocks, send=False)[0][0]
            if GetShortID(tx, self.compute_full_salt()) < BISEC_MEDIAN:
                local_txs.append(tx)
        more_remote_txs = []
        self.blocks_offset += 60
        self.nodes[0].p2ps[0].send_txs_and_test(remote_txs, self.nodes[0], success = True)
        self.proceed_in_time(RECON_REQUEST_INTERVAL + 1)
        self.receive_reqreconcil(self.test_node, expected_set_size=len(remote_txs))
        self.transmit_sketch(self.test_node, txs_to_sketch=local_txs, bisection=False,
            capacity=30, expected_announced_txs=[])
        ...
        self.handle_reconciliation_finalization(self.test_node, expected_success=ReconResult.RECON_LOW_FAILED,
        ...
        self.handle_reconciliation_finalization(self.test_node, expected_success=ReconResult.RECON_HIGH_FAILED,
        ...
        self.transmit_sketch(self.test_node, txs_to_sketch=[], bisection=False,
        ...
        self.handle_bisection_request(self.test_node)
        more_remote_txs.extend(self.generate_txs(0, 1, 0, blocks, self)[1])
        self.transmit_sketch(self.test_node, txs_to_sketch=[], bisection=True, capacity=0,
    ...
    def check_remote_recon_set_outgoing(self, txs_to_expect, delay=RECON_REQUEST_INTERVAL + 1):
        ...
        self.transmit_sketch(self.test_node, txs_to_sketch=[], bisection=False, capacity=0, expected_announced_txs=txs_to_expect)
        self.handle_reconciliation_finalization(self.test_node, expected_success=ReconResult.RECON_FAILED,
            expected_requested_txs=[], after_bisection=False)
